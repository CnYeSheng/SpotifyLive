...
class EnhancedStorage {
    ...
    async getSongSettings(trackId) {
        ...
        try {
            if (this.dbType === 'postgres') {
                ...
                if (this.localData && this.localData[trackId]) {
                    settings = this.localData[trackId];
                }
            }
        } catch (e) {
    ...
    async saveSongSettings(trackId, settings) {
        ...
        try {
            ...
            if (this.dbType === 'postgres') {
                ...
                this.localData[trackId] = updated;
                // Async save to file
    ...
    async getAllLyrics() {
        ...
        try {
            if (this.dbType === 'postgres') {
                const res = await this.db.query('SELECT * FROM song_settings');
                ...
                const [rows] = await this.db.execute('SELECT * FROM song_settings');
                ...
                if (this.localData) {
                    allData = Object.entries(this.localData).map(([key, value]) => ({
                        trackId: key,
                        ...value
                    }));
                }
